...
dd <- corr["ds2ref_Fibromyocyte",] %>% reshape2::melt()
dd$x <- rownames(dd)
tfunc <- function(x){
  s <- strsplit(x,"_")[[1]][1]
}
...
ggplot(dd) + geom_bar(aes(x,weight = value, fill = x)) + bartheme + scale_y_continuous(breaks = seq(0.7, 1, 0.1)) + scale_fill_manual(values = colors_list) + coord_cartesian(ylim= c(0.7,1))
