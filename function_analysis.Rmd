...
cnetplot(up_enrich.go, showCategory = 8, colorEdge = T)
emapplot(up_enrich.go, showCategory = 10)
...
down_enrich.go <- enrichGO(
    ...
    ont = "ALL", # 可选 BP、MF、CC，也可以指定 ALL 同时计算 3 者
    pAdjustMethod = "fdr", pvalueCutoff = 0.05, qvalueCutoff = 0.2)
ggsave("down_enrich1.svg",device = svg, plot = dotplot(down_enrich.go, showCategory = 10), width = 10, height = 6)

ggsave("down_enrich.svg",device = svg, plot = cnetplot(down_enrich.go,  showCategory = 8, colorEdge = T), width = 10, height = 6)
emapplot(down_enrich.go, showCategory = 10)
...
# BMP通路相关
```{r}
Dotplot(c("BMP","BMPR1A","BMPR1B"),ds2)
...
colors_list[1:3]
colors_border <- colors_list[1:3]
colors_in <- aero_colors_list[1:3]
